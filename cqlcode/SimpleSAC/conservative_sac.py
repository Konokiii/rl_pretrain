...
class ConservativeSAC_REG(ConservativeSAC):
    def __init__(self, config, policy, qf1, qf2, target_qf1, target_qf2, variant):
        super().__init__(config, policy, qf1, qf2, target_qf1, target_qf2, variant)
        optimizer_class = {
            'adam': torch.optim.Adam,
            'sgd': torch.optim.SGD,
        }[self.config.optimizer_type]
        ...
        self.qf_optimizer = optimizer_class(
            ...
            lr=self.config.qf_lr, weight_decay=variant['weight_decay']
        )
    ...
    def pretrain(self, batch, pretrain_mode):
        ...
        if pretrain_mode in ['syn_mdp', 'offline_centroid', 'offline_self']:
            obs_next_q1 = self.qf1.predict_next_obs(observations, actions)
            obs_next_q2 = self.qf2.predict_next_obs(observations, actions)
            pretrain_loss1 = F.mse_loss(obs_next_q1, next_observations)
            pretrain_loss2 = F.mse_loss(obs_next_q2, next_observations)
    ...
    def train(self, batch, bc=False, safe_q_max=None, extra_q_loss=None, extra_q_loss_discount=1):
        self._total_steps += 1
        ...
        if not self.config.use_cql:
            ...
            conservative_loss = cql_min_qf1_loss + cql_min_qf2_loss
            qf_loss = qf1_loss + qf2_loss + conservative_loss

        # Modified: Add synthetic regularization loss here:
        if extra_q_loss:
            qf_loss += extra_q_loss_discount * extra_q_loss
